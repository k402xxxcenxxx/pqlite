...
def server():
    ...
    server_instance = DistributedDatabaseServer('localhost', 0)  # Use port 0 to allow the OS to pick an available port
...
def test_server_accepts_connections(server):
    ...
    host, port = server.server_socket.getsockname()  # Retrieve the port assigned by the OS
